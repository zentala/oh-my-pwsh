BeforeAll {
    ...
    . $PSScriptRoot/../../modules/cc/main.ps1

    # Override config paths to use TestDrive (AFTER sourcing so we override module defaults)
    $script:CcConfigDir = Join-Path $TestDrive 'cc'
    $script:CcConfigFile = Join-Path $TestDrive 'cc' 'config.json'
    $script:CcPlansDir  = Join-Path $TestDrive 'cc' 'plans'
...
Describe "_cc_plan_generate_id" {
    ...
    Context "When plan with same time exists" {
        BeforeAll {
            ...
            $existingFile = Join-Path $script:CcPlansDir 'plan-20260414-0200.json'
        ...
        AfterAll {
            Remove-Item (Join-Path $script:CcPlansDir 'plan-20260414-0200.json') -Force -ErrorAction SilentlyContinue
        }
    }
}
    ...
    Context "When plans exist" {
        BeforeAll {
            $plan1 = @{
                ...
                taskName = 'cc-plan-test-001'; completedAt = $null
                exitCode = $null; logFile = 'plan-test-001.log'
                outputFile = 'plan-test-001.output.md'
            }
            $plan1 | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $script:CcPlansDir 'plan-test-001.json')
            ...
            $plan2 = @{
                ...
                taskName = 'cc-plan-test-002'; completedAt = '2026-04-14T02:30:00'
                exitCode = 0; logFile = 'plan-test-002.log'
                outputFile = 'plan-test-002.output.md'
            }
            $plan2 | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $script:CcPlansDir 'plan-test-002.json')
        }
        ...
        AfterAll {
            Remove-Item (Join-Path $script:CcPlansDir 'plan-test-001.json') -Force -ErrorAction SilentlyContinue
            Remove-Item (Join-Path $script:CcPlansDir 'plan-test-002.json') -Force -ErrorAction SilentlyContinue
        }
    }
}
...
Describe "_cc_plan_create" {
    ...
    Context "When creating with --at flag" {
        It "Creates plan JSON file" {
            ...
            _cc_plan_create -SubArgs @('test plan prompt', '--at', $atTime)

            $planFiles = Get-ChildItem -Path $script:CcPlansDir -Filter 'plan-*.json'
            $planFiles.Count | Should -BeGreaterOrEqual 1
        }

        It "Stores correct prompt in JSON" {
            $planFiles = Get-ChildItem -Path $script:CcPlansDir -Filter 'plan-*.json' | Sort-Object LastWriteTime -Descending
    ...
    Context "When creating with --auto-edit" {
        It "Sets autoEdit to true" {
            ...
            _cc_plan_create -SubArgs @('edit plan', '--at', $atTime, '--auto-edit')
    ...
    Context "When creating with --resume" {
        It "Stores resume session ID" {
            ...
            _cc_plan_create -SubArgs @('resume msg', '--at', $atTime, '--resume', 'session-xyz')
    ...
    Context "When creating with --timeout" {
        It "Stores custom timeout" {
            ...
            _cc_plan_create -SubArgs @('long task', '--at', $atTime, '--timeout', '120')
    ...
    Context "When claude is not installed" {
        ...
        It "Shows error and does not create plan" {
            $beforeCount = (Get-ChildItem -Path $script:CcPlansDir -Filter 'plan-*noclaude*.json' -ErrorAction SilentlyContinue).Count
            _cc_plan_create -SubArgs @('noclaude plan', '--at', '23:59')
    ...
    Context "When --at time format is invalid" {
        ...
        It "Shows error for bad format" {
            _cc_plan_create -SubArgs @('bad time', '--at', 'abc')
    ...
    AfterAll {
        ...
        Get-ChildItem -Path $script:CcPlansDir -Filter 'plan-*.json' -ErrorAction SilentlyContinue |
            Remove-Item -Force
    }
}
    ...
    Context "When plans exist" {
        BeforeAll {
            @{
                ...
                createdAt = '2026-04-13T22:00:00'; autoEdit = $false
                resumeSession = $null; timeoutMinutes = 60
                taskName = 'cc-plan-list-test'; completedAt = $null
                exitCode = $null; logFile = 'plan-list-test.log'
                outputFile = 'plan-list-test.output.md'
            } | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $script:CcPlansDir 'plan-list-test.json')
        }
        ...
        AfterAll {
            Remove-Item (Join-Path $script:CcPlansDir 'plan-list-test.json') -Force -ErrorAction SilentlyContinue
        }
    }
}
    ...
    Context "When plan exists" {
        BeforeAll {
            @{
                ...
                taskName = 'cc-plan-show-test'
                completedAt = '2026-04-14T01:45:00'; exitCode = 0
                logFile = 'plan-show-test.log'; outputFile = 'plan-show-test.output.md'
            } | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $script:CcPlansDir 'plan-show-test.json')
        }

        It "Displays plan details" {
            _cc_plan_show -Id 'show-test'
        ...
        AfterAll {
            Remove-Item (Join-Path $script:CcPlansDir 'plan-show-test.json') -Force -ErrorAction SilentlyContinue
        }
    }

    Context "When plan does not exist" {
        It "Shows error" {
            _cc_plan_show -Id 'nonexistent'
    ...
    Context "When cancelling a pending plan" {
        BeforeAll {
            @{
                ...
                createdAt = '2026-04-13T22:00:00'; autoEdit = $false
                resumeSession = $null; timeoutMinutes = 60
                taskName = 'cc-plan-cancel-test'; completedAt = $null
                exitCode = $null; logFile = 'plan-cancel-test.log'
                outputFile = 'plan-cancel-test.output.md'
            } | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $script:CcPlansDir 'plan-cancel-test.json')
        }

        It "Updates status to cancelled" {
            _cc_plan_cancel -Id 'cancel-test'

            $plan = Get-Content (Join-Path $script:CcPlansDir 'plan-cancel-test.json') -Raw | ConvertFrom-Json
        ...
        AfterAll {
            Remove-Item (Join-Path $script:CcPlansDir 'plan-cancel-test.json') -Force -ErrorAction SilentlyContinue
        }
    }
    ...
    Context "When plan is already completed" {
        BeforeAll {
            @{
                ...
                createdAt = '2026-04-13T22:00:00'; autoEdit = $false
                resumeSession = $null; timeoutMinutes = 60
                taskName = 'cc-plan-done-test'
                completedAt = '2026-04-14T01:30:00'; exitCode = 0
                logFile = 'plan-done-test.log'; outputFile = 'plan-done-test.output.md'
            } | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $script:CcPlansDir 'plan-done-test.json')
        }

        It "Shows warning instead of cancelling" {
            _cc_plan_cancel -Id 'done-test'
            Should -Invoke Write-Host -ParameterFilter { $Object -like '*already completed*' }
        }

        AfterAll {
            Remove-Item (Join-Path $script:CcPlansDir 'plan-done-test.json') -Force -ErrorAction SilentlyContinue
        }
    }

    Context "When plan does not exist" {
        It "Shows error" {
            _cc_plan_cancel -Id 'ghost'
            Should -Invoke Write-Host -ParameterFilter { $Object -like '*Plan not found*' }
        }
    }
}
    ...
    Context "When old completed plans exist" {
        BeforeAll {
            ...
            @{
                ...
                resumeSession = $null; timeoutMinutes = 60
                taskName = 'cc-plan-old-plan'
                completedAt = '2026-04-01T01:30:00'; exitCode = 0
                logFile = 'plan-old-plan.log'; outputFile = 'plan-old-plan.output.md'
            } | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $script:CcPlansDir 'plan-old-plan.json')
            ...
            @{
                ...
                resumeSession = $null; timeoutMinutes = 60
                taskName = 'cc-plan-recent-plan'; completedAt = $null
                exitCode = $null; logFile = 'plan-recent-plan.log'
                outputFile = 'plan-recent-plan.output.md'
            } | ConvertTo-Json -Depth 5 | Set-Content (Join-Path $script:CcPlansDir 'plan-recent-plan.json')
        }

        It "Removes old completed plan" {
            _cc_plan_clean
            Test-Path (Join-Path $script:CcPlansDir 'plan-old-plan.json') | Should -Be $false
        }

        It "Keeps recent pending plan" {
            Test-Path (Join-Path $script:CcPlansDir 'plan-recent-plan.json') | Should -Be $true
        }

        AfterAll {
            Remove-Item (Join-Path $script:CcPlansDir 'plan-recent-plan.json') -Force -ErrorAction SilentlyContinue
        }
    }
...
Describe "_cc_plan_dispatch" {
    Context "When called with no args" {
        It "Shows help" {
            _cc_plan_dispatch -SubArgs @()
            Should -Invoke Write-Host -ParameterFilter { $Object -like '*cc plan*schedule Claude*' }
        }
    }

    Context "When called with 'help'" {
        It "Shows help" {
            _cc_plan_dispatch -SubArgs @('help')
            Should -Invoke Write-Host -ParameterFilter { $Object -like '*cc plan*schedule Claude*' }
        }
    }
    ...
    Context "When called with 'list'" {
        It "Does not throw" {
            { _cc_plan_dispatch -SubArgs @('list') } | Should -Not -Throw
        }
    }
}
...
Describe "cc/plan-daemon.ps1" {
    It "Has valid PowerShell syntax" {
        $daemonPath = Join-Path $PSScriptRoot '../../scripts/cc/plan-daemon.ps1'
        $errors = $null
